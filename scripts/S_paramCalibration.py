...
class VNACalibration:
    ...
    def connect(self) -> bool:
        ...
        try:
            ...
            self.ins_odc = rm.open_resource(self.ip_address)
            self.ins_odc.timeout = 10000  # 10秒超时
            
            # 测试连接
            response = self.ins_odc.query('*IDN?')
            self.logger.info(f"仪器标识: {response.strip()}")
    ...
    def send_command(self, command: str) -> Optional[str]:
        ...
        Returns:
            Optional[str]: 仪器的响应，如果出错则返回None
        """
        if not self.is_connected:
            self.logger.error("仪器未连接")
            return None
        
        try:
            if '?' in command:
                return self.ins_odc.query(command).strip()
            else:
                self.ins_odc.write(command)
                return "OK"
        except Exception as e:
    ...
    def perform_calibration(self):
        ...
        # 测量开路标准件 (Open)
        input("请连接端口1到开路，端口2到负载，然后按回车继续...")
        ...
        # 测量短路标准件 (Short)
        input("请连接端口1到短路，端口2到负载，然后按回车继续...")
        ...
        # 测量端口2开路，端口1负载
        input("请连接端口2到开路，端口1到负载，然后按回车继续...")
        ...
        # 测量端口2短路，端口1负载
        input("请连接端口2到短路，端口1到负载，然后按回车继续...")
    ...
    def plot_results(self):
        ...
        # S11
        plt.subplot(2, 2, 1)
        plt.plot(x, 20 * np.log10(np.abs(self.results['S11'])))
        plt.title('S11')
        plt.ylabel('dB')
        plt.grid(True)
        ...
        # S21
        plt.subplot(2, 2, 2)
        plt.plot(x, 20 * np.log10(np.abs(self.results['S21'])))
        plt.title('S21')
        plt.ylabel('dB')
        plt.grid(True)
        ...
        # S12
        plt.subplot(2, 2, 3)
        plt.plot(x, 20 * np.log10(np.abs(self.results['S12'])))
        plt.title('S12')
        plt.ylabel('dB')
        plt.grid(True)
        ...
        # S22
        plt.subplot(2, 2, 4)
        plt.plot(x, 20 * np.log10(np.abs(self.results['S22'])))
        plt.title('S22')
        plt.ylabel('dB')
        plt.grid(True)
    ...
    def run_full_calibration_and_measurement(self):
        """运行完整的校准和测量流程"""
        if not self.connect():
            return
