...
class DataExportWorker(QObject):
    ...
    def calculate_total_steps(self):
        ...
        if self.export_config.get('export_data', True):
            ...
            if self.export_config.get('export_additional_data', True):
                ...
                total += len(channels) * 7  # 每个通道7种额外数据
        
        # 图片步骤
        if self.export_config.get('export_images', True):
            plot_types = self.export_config.get('plot_types', [])
            total += len(plot_types)
    ...
    def export_data_files(self, current_step, total_steps):
        ...
        try:
            # 根据文件类型导出数据
            if file_ext == '.csv':
                self.export_csv_data(base_path, file_path)
            elif file_ext == '.json':
            ...
            current_step += 1
            self.progress.emit(current_step, total_steps, "数据文件导出完成")
            
            # 导出额外数据
            if self.export_config.get('export_additional_data', True):
                current_step = self.export_additional_data(base_path, current_step, total_steps)
    ...
    def export_csv_data(self, base_path, file_path):
        """导出CSV数据"""
        ...
        if results and averages and config:
            ...
            # 导出额外CSV数据
            self.export_additional_csv_data(base_path, results, averages, config)
        else:
            # 如果没有分析数据，保存基本的文本结果
            self.export_text_data(file_path)

    def export_additional_csv_data(self, base_path, results, averages, config):
        """导出额外的CSV数据"""
        channels = self.export_config.get('channels', ['adc1', 'adc2'])
        ...
        for channel in channels:
            if channel in results and channel in averages:
                ...
                # 保存时域数据
                self.save_time_domain_data(base_path, channel, channel_results, channel_averages, config, channel_suffix)
                
                # 保存频域数据
                self.save_frequency_domain_data(base_path, channel, channel_results, channel_averages, config, channel_suffix)
                
                # 保存差分数据
                self.save_differential_data(base_path, channel, channel_results, channel_averages, config, channel_suffix)
                
                # 保存边沿分析结果
                self.save_edge_analysis_data(base_path, channel, channel_results, channel_suffix)

    def save_time_domain_data(self, base_path, channel, results, averages, config, channel_suffix):
        """保存时域数据"""
    ...
    def save_frequency_domain_data(self, base_path, channel, results, averages, config, channel_suffix):
        """保存频域数据"""
    ...
    def save_differential_data(self, base_path, channel, results, averages, config, channel_suffix):
        """保存差分数据"""
        # 差分时域数据
    ...
    def save_edge_analysis_data(self, base_path, channel, results, channel_suffix):
        """保存边沿分析结果"""
    ...
    def export_additional_data(self, base_path, current_step, total_steps):
        """导出额外数据（占位方法，具体实现在主类中）"""
        # 这个方法的具体实现在DataAnalysisController中
        # 这里只是占位，实际的额外数据导出在export_additional_csv_data中完成
        return current_step
